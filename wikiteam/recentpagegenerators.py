...
class GeneratorFactory(object):
    ...
    def handleArg(self, arg):
        ...
        elif arg.startswith('-recentchangessince'):
          name ='-recentchangessince'


          gen = RecentchangesSincePageGenerator(since = (arg[len(name)+1:]))
...
def RecentchangesSincePageGenerator(since=None, site=None):
    ...
    for item in site.recentchanges(number=1000, rcstart=since, rctype = 'edit'):
        yield item
        ...
        for arg in pywikibot.handleArgs():
            ...
            if gen:
                ...
                for page in gen:
                    ...
                    #pywikibot.output("%4d: %s %s" % (i, page.title(), page.editTime()), toStdout = True)
                    #text = 
                    #page.get()
                    #pywikibot.output("%4d: %s" % (i, text), toStdout = True)
                    pywikibot.output("%4d: %s %s" % (i, page.title(), "time"), toStdout = True)
