...
#
__version__='$Id: date.py 10186 2012-05-05 17:14:00Z xqt $'
...
#        startClock = time.clock()
        for value in xrange(start, stop, step):
            try:
                ...
                pywikibot.output(u"********** Error in date.formats['%s']['%s'](%d)" % (formatName, code, value))
                raise
#        pywikibot.output( u"%s\t%s\t%f" % (formatName, code, time.clock() - startClock) )
...
def test(quick=False, showAll=False):
    ...
    if quick:
        #pywikibot.output(u'Date module quick consistency test passed')
        pass
    else:
